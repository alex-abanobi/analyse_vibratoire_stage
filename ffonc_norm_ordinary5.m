function Z_fen_group = ffonc_norm_ordinary5(Z_fen_group)
...
%         numb = [23, 29, 32, 36, 38, 42, 44, 53, 57, 64];
        ...
        for i = 5:5:85
            Z_fen_group(:,1:i) = bsxfun(@rdivide,Z_fen_group(:,1:i),abs(Z_fen_group(:,i))); %Z_fen_group(:,1:i)./(Z_fen_group(:,i)); % 12 is 13
            Z_fen_group(:,i+1:i+5) = bsxfun(@rdivide,Z_fen_group(:,i+1:i+5),abs(Z_fen_group(:,i+1))); %Z_fen_group(:,i+1:i+6)./(Z_fen_group(:,i+1));
        end

        Z_fen_group(:,1:90) = bsxfun(@rdivide,Z_fen_group(:,1:90),abs(Z_fen_group(:,90))); % Z_fen_group(:,1:108)./(Z_fen_group(:,108)); 
        Z_fen_group(:,91:93) = bsxfun(@rdivide,Z_fen_group(:,91:93),abs(Z_fen_group(:,91))); %Z_fen_group(:,109:112)./(Z_fen_group(:,109));
        Z_fen_group(:,1:93) = bsxfun(@rdivide,Z_fen_group(:,1:93),abs(Z_fen_group(:,93))); %Z_fen_group(:,1:112)./(Z_fen_group(:,112)); 
