...
dd=1;
for jjc = 1:3:size(Z,2) % 1 for x

    F_fen(:,dd) = fft(ZR(:,jjc)); % take the fourier transform of 
    dd = dd + 1;
end % go to the next column of that same direction


%%%% external normalisation %%%%
F_fen_group = fonc_fish(F_fen); % fish out the bad captors
F_fen_group_uf = F_fen_group;
F_fen_group = ffonc_norm_ordinary5(F_fen_group);
...
save('D:\abanobi\Documents\STAGEISTERRE\gueguen\Archive\F_fen_group_CS1_X_saved.mat','F_fen_group');
