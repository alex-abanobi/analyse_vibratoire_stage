...
Z_used_fen_group = zeros(12000,size(Z,2)/3);
...
dd=1;
for jjc = 1:3:size(Z,2) % 1 for x, 2 for y, 3 for z
c = 1;
cc = 12000;
for i=1:29
...
%Z_used_fen_group = fonc_fish2(Z_used_fen_group); % fish out the bad captors
...
figure
% cd1 = linspace(0,91,12001); % frequency
% cd2 = 0:200/12000/60:200/60; % captor number

cd1 = 0:(1/60):(12000/60);
cd2 = linspace(0,size(Z_used_fen_group,2),12001);

% cd1 = linspace(0,93,162000001);
% cd2 = 0:200/180000:180000;
plot(cd1,cd2);
clims = [0 10];
h = imagesc((Z_fen_group)/1.2, clims);
hold on
