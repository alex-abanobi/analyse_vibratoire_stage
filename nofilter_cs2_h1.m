...
Z_fen_group = zeros(180000,size(ZZ,2)/3);
G_fen_group = zeros(180000,size(ZZ,2)/3);
...
wpe_x = 1;
ZR = zeros(size(ZZ,1),size(ZZ,2));  % this will put it back in x y z form because before it was in z x y
for wd_x = 2:3:size(ZZ,2)
    ZR(:,wpe_x) = Z(:,wd_x);
    wpe_x = wpe_x + 3;
end
...
ZR = fonc_fish2_big(ZR); % fish out the bad captors
